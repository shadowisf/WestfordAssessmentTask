...
export default function RelatedCourses() {
  return (
    <div className="px-[42px] pb-[69px] space-y-[47px] pb-[69px]">
      ...
      <section className="grid grid-cols-4 gap-[40px] place-items-center">
        <RelatedCoursesItem
...
function RelatedCoursesItem({
  ...
  return (
    <a
      href="#"
      className="group relative w-[308px] h-[363px] rounded-[25px] bg-[#F5F5F5] flex flex-col overflow-hidden"
    >
      {/* description */}
      <section className="p-[15px] space-y-[16px] z-[2] relative h-[172px] group-hover:p-[25px] transition-padding duration-300 ease-out">
        <img className="w-[150px]" src={ImagePearsonLogo} />
        <span className="font-light text-[18px]">{children}</span>
      </section>

      {/* gradient and button */}
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 z-[3] w-full h-[57px] pointer-events-none mt-[19px] bg-[linear-gradient(to_bottom,_rgb(245_245_245/1)_0%,_rgb(247_247_247/0.8)_39%,_rgb(247_247_247/0)_100%)] flex justify-end group-hover:opacity-0 transition-opacity duration-300 ease-out">
        <button className="w-[67px] h-[28px] bg-[#ECA22D] rounded-[15px] flex items-center justify-center ps-[26px] pe-[8px] me-[9px]">
          <ArrowRightLongerA className="text-white" />
        </button>
      </div>

      <div className="relative z-[1]">
        <div className="absolute inset-0 text-[12px] z-[-1] p-[25px]">
          {/* course stats grid */}
          <div className="grid grid-cols-2 gap-y-[23px] gap-x-[30px]">
        ...
        <img
          src={img}
          className="w-full h-full transition-opacity duration-300 ease-out group-hover:opacity-0"
        />
      </div>
    </a>
  );
